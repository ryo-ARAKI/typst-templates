...
`title` は図側の見出し，`caption-title` はガイド側の見出しです．`caption-title` の既定値は `[Guide]` で，`caption-title: none` にするとガイド側の見出しを非表示にできます．`widths` は `(左列幅, 右列幅)` の物理順で渡し，未指定時は `figure-side` 側の図列が広くなります．
